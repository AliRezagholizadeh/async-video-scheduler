...
def main():
    ...
    schedule["program_schedule"]["start_time"] = (start:= datetime.datetime.now() + datetime.timedelta(seconds = 10)).time().strftime(format=TIME_FORMAT)
    for ontime_indx in range(len((ontime_contents:= schedule["program_schedule"]["on-time_contents"]))):
        if(ontime_contents[ontime_indx]["name"] == "start_program"):
            ontime_contents[ontime_indx]["play_time"]= start.time().strftime(format=TIME_FORMAT)
        else:
            ontime_contents[ontime_indx]["play_time"] = (start + datetime.timedelta(seconds= 20)).time().strftime(format=TIME_FORMAT)
    ...
    program_pipeline(config = config, program_schedule = schedule)
